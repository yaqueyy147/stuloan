package com.stuloan.web.interceptor;
...
//        System.out.println("*********\n当前路径-->" + currentUrl + "\n************");
        ...
        if(isConsoles(currentUrl)){//后台用户
            sysuser = Userutils.getcookieuser(httpServletRequest,Userutils.CONSOLE_COOKIE_NAME);
            type = 1;
        }else{//前台用户
            sysuser = Userutils.getcookieuser(httpServletRequest,Userutils.FRONG_COOKIE_NAME);
            type = 2;
        }

        if(!CommonUtil.isBlank(sysuser) && !CommonUtil.isBlank(sysuser.getId())){
            return true;
        }
